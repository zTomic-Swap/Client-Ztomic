  ...
  const filteredAndSortedOrders = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      if (sortBy === "newest") {
        ...
        const aInterested = a.interestedParties.reduce((total, party) => total + party.identity.length, 0)
        const bInterested = b.interestedParties.reduce((total, party) => total + party.identity.length, 0)
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {order.interestedParties.reduce((total, party) => total + party.identity.length, 0)} interested{" "}
                        {order.interestedParties.reduce((total, party) => total + party.identity.length, 0) !== 1 ? "parties" : "party"}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2 ml-4">
                      <Button
                        ...
                        title={`Show interest on ${selectedChain}`}
                      >
